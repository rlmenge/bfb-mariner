...
ADD create_bfb .
ADD update.cap .
ADD selfhost.repo /etc/yum.repos.d/

ENV kernel=5.15.2.1-2.cm2
...
RUN dnf install -y gpio-mlxbf ibacm infiniband-diags infiniband-diags-compat ipmb-dev-int kernel-mft knem knem-modules libibumad libibverbs libibverbs-utils librdmacm librdmacm-utils mlnx-tools mlx-bootctl mlx-trio mlxbf-bootctl mlxbf-bootimages mlxbf-livefish mlxbf-pmc mstflint ofed-scripts opensm opensm-devel opensm-libs opensm-static perftest rdma-core rdma-core-devel srp_daemon tmfifo ucx ucx-cma ucx-devel ucx-ib ucx-rdmacm 
